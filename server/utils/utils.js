...
module.exports = {
    ...
    sendResponseForAPI : function(err,req,res,data) {
        ...
        if (err) {
            if (_.has(err,'acedInterventionStatusCode')) {
                res.statusCode = err.acedInterventionStatusCode;
                if (_.isObject(err.acedInterventionStatusMessage)) {
                    statusMessage = _.values(err.acedInterventionStatusMessage)
                    // _.values returns an array
                    if (_.isArray(statusMessage)) {
                        statusMessage = _.flatten(statusMessage)
                    }
                } else {
                    statusMessage = err.acedInterventionStatusMessage
                }
                res.setHeader('x-acedIntervention-status',statusMessage);
                dslog.error({url:req.route,method:req.method},{statusCode:res.statusCode,statusMessage:statusMessage});
            } else {
                res.statusCode = 500;
                res.setHeader('x-acedIntervention-status','Critical Error')
            ...
            res.setHeader('x-acedIntervention-status', 'Success');
            dslog.info({url: req.route, method: req.method}, {statusCode: res.statusCode, statusMessage: 'Success'});
            res.send(data ? data : '');
        }
    ...
    formatAccountRow : function(accountRow,profileRow) {
        var location,cityState,leadPictureUrl=null,incompleteProfile=true;
        if (profileRow && profileRow.zipCode) {
            location = zipcodeModel.lookup(profileRow.zipCode);
            if (location) {
                cityState= location.city+" , "+location.state_short;
            }
        }
        if (profileRow) {
            leadPictureUrl = profileRow.leadPicture ? profileRow.leadPicture.url : null
        }

        if (profileRow && profileRow.stepsCompleted == 3) {
            incompleteProfile = false
        }

        return {
            _id : accountRow._id,
            age: profileRow ? profileRow.age : null,
            email: accountRow.email,
            gender: profileRow ? profileRow.gender : null,
            userName: accountRow.userName,
            zipCode: profileRow ? profileRow.zipCode : null,
            cityState : cityState ? cityState : null,
            authResponse:accountRow.authResponse,
            loginMethod : accountRow.loginMethod || 'acedIntervention',
            leadPictureUrl : leadPictureUrl,
            incompleteProfile : incompleteProfile,
            matchMakeOnly : accountRow.matchMakeOnly,
            stepsCompleted : profileRow ? profileRow.stepsCompleted : null,
            tooltipsDisplayed : accountRow ? accountRow.tooltipsDisplayed : null
        };
    },

    formatLeadPicture : function (leadPicture) {
        return leadPicture;
    },

    isInteger : function(x) { return Math.floor(x) === x; },
